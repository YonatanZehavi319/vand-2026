...
def main() -> None:
    ...
    # INP-Former args
    parser.add_argument('--inp_save_dir', type=str, default='weights', help='INP-Former weights dir (default: weights)')
    parser.add_argument('--tiling', action='store_true', default=True, help='Use tiling for INP-Former (default: True)')
    parser.add_argument('--no_tiling', action='store_false', dest='tiling', help='Disable tiling')
    parser.add_argument('--target_tile', type=int, default=600, help='Target tile size (default: 600)')
    parser.add_argument('--batch_size', type=int, default=4)
    # CPR args
    parser.add_argument('--cpr_save_dir', type=str, default=None, help='CPR output dir (default: {out_dir}/cpr)')
    parser.add_argument('--cpr_checkpoints', type=str, default='log/mvtec_train_v2/{category}/03000.pth', help='CPR checkpoint pattern')
    # Validation dirs
    parser.add_argument('--inp_val_dir', type=str, default='weights/INP-Former-Multi-Class_dataset=MVTec-AD_Encoder=dinov2reg_vit_base_14_Resize=448_Crop=392_INP_num=6/val_heatmaps', help='INP-Former validation heatmaps dir')
    parser.add_argument('--cpr_val_dir', type=str, default='output/cpr/val_heatmaps', help='CPR validation heatmaps dir')
    # Smoothing options
    ...
    parser.add_argument('--guided', action='store_true', default=True, help='Apply guided filter (default: True)')
    parser.add_argument('--no_guided', action='store_false', dest='guided', help='Disable guided filter')
    parser.add_argument('--guided_r', type=int, default=8)
    parser.add_argument('--guided_eps', type=float, default=0.001)
    # Adaptive FDR
    ...
    # Auto CPR weight
    parser.add_argument('--auto_cpr_weight', action='store_true', default=True, help='Auto-compute per-category CPR weight from EdgeCorr (default: True)')
    parser.add_argument('--no_auto_cpr_weight', action='store_false', dest='auto_cpr_weight', help='Disable auto CPR weight')
    ...
    parser.add_argument('--cpr_power', type=float, default=1.5, help='Power for CPR signal in boost mode (default: 1.5)')
